      ...
      <section className="px-4 py-20">
        <div className="mx-auto grid max-w-[1180px] gap-12 lg:grid-cols-[.95fr_1.05fr]">
          ...
          <div>
            ...
            <h1 className="mt-5 text-balance text-5xl font-black tracking-[-0.055em] text-white sm:text-7xl">
              {product.name}
            </h1>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {[product.condition, product.stock, product.warranty].map((value) => (
                <span
                  key={value}
                  className="rounded-full border border-slate-700 bg-slate-950/50 px-3 py-2 text-xs font-black text-slate-200"
                >
                  {value}
                </span>
              ))}
            ...
            <div className="surface mt-6 rounded-3xl p-6">
              ...
              <p className="mt-2 text-xs text-slate-500">
                Confirm exact configuration, availability and final price with the
                store.
              </p>
            </div>
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              ...
              <Link
                href={`/compare?ids=${product.slug},${related[0]?.slug ?? products[0].slug}`}
            ...
            <div className="mt-8 overflow-hidden rounded-3xl border border-slate-700">
              {Object.entries(rows)
                ...
                .map(([label, value]) => (
                  <div
                    key={label}
                    className="grid grid-cols-[140px_1fr] border-b border-slate-700 last:border-b-0 sm:grid-cols-[190px_1fr]"
                  >
                    <strong className="bg-slate-950/45 p-4 text-sm text-slate-400">
                      {label}
                    </strong>
            ...
            <h2 className="mt-8 text-2xl font-black text-white">
              Included with this demo product
            </h2>
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              {product.included.map((item) => (
                <div key={item} className="surface rounded-2xl p-4 text-sm text-slate-300">
                  <Icon name="check" className="mb-3 size-5 text-emerald-300" />
                  {item}
                </div>
              ))}
            </div>
